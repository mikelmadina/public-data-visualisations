...
emaitzak2 <- emaitzak |> 
  ...
  mutate(alderdia = fct_relevel(alderdia, "eaj_pnv", "eh_bildu", "pse_ee_psoe", "pp", "sumar", "vox"),
         ...
         naziotasuna_percent = case_when(
           naziotasuna == "Besteak" ~ percent_b_hautagaitzari * -1,
           .default = percent_b_hautagaitzari
         ),
...
naziotasuna_datuak <- emaitzak2 |> 
  filter(naziotasuna == "Abertzalea") |> 
  group_by(udalerria) |> 
  summarise(guztira = sum(naziotasuna_percent)) |> 
...
y_ardatzerako_datuak <- emaitzak2 |> 
  ...
  mutate(udalerria = fct_relevel(as.factor(udalerria), levels(naziotasuna_datuak$udalerria))) |> 
  arrange(udalerria) |> 
...
x_ardatzerako_datuak <- emaitzak2 |> 
    mutate(udalerria = fct_relevel(as.factor(udalerria), levels(naziotasuna_datuak$udalerria)),
           alderdia = fct_relevel(as.factor(alderdia), "sumar", "pse_ee_psoe", "pp", "vox",  "eaj_pnv", "eh_bildu" )) |> 
    arrange(udalerria) |> 
    group_by(udalerria, naziotasuna) |> 
    arrange(alderdia) |> 
    mutate(
      prob = abs(naziotasuna_percent),
      xmax = cumsum(prob),
      xmin = xmax - prob
      ) |> 
    ungroup() |> 
    mutate(xmax = case_when(
      naziotasuna == "Besteak" ~ xmax * -1,
      .default = xmax),
      xmin = case_when(
        naziotasuna == "Besteak" ~ xmin * -1,
      .default = xmin)) |> 
  group_by(udalerria) |> 
...
p_naziotasuna <- ggplot(datuak) +
  ...
  # titulu-legendak
  annotate(geom = "richtext", x = -25, y = max(y_ardatzerako_datuak$ymax) + 35, label = glue(
    '<span style="font-size: 18pt;">**Beste alderdiak**</span>',
    '<br><br>',
  ...
  annotate(geom = "richtext", x = 50, y = max(y_ardatzerako_datuak$ymax) + 35, label = glue(
    '<span style="font-size: 18pt;">**Alderdi abertzaleak**</span>',
    '<br><br>',
  ...
  annotate(geom = "text", x = -50 , y = max(legendarako_ymax) + 20, label = "Erroldatutakoak", size = 5, family = tipografia, fontface = "bold", hjust = 0) +
  annotate(geom = "rect", xmin = -30, xmax = -50, ymin = legendarako_ymin, ymax = legendarako_ymax, fill = "grey85", color = "black") +
  annotate(geom = "text", x = -29, y = legenda_etiketa, label = levels(emaitzak2$nat_breaks_errolda), hjust = 0, size = 2) +
  # Testuak
  labs(title = "2024 Eusko Legebiltzarrerako Hauteskundeak",
       subtitle = "Udalerrietako naziotasun espektroa",
       caption = glue(
...
ggsave(
  here("2024-04-22-EAE_Hauteskundeak", "espektroa_naziotasuna.png"),
  plot = p_naziotasuna,
  device = "png",
  width = 42,
  height = 59.4,
...
ggsave(
  here("2024-04-22-EAE_Hauteskundeak", "espektroa_naziotasuna.pdf"),
  plot = p_naziotasuna,
  device = cairo_pdf,
  width = 42,
  height = 59.4,
